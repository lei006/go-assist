...
//新建session
func NewSession(server *RtspServer, conn net.Conn) *Session {
		...
		if buf1[0] == 0x24 { //rtp data
			...
			case session.aRTPChannel:
				...
				if elapsed >= 30*time.Second {
					//beego.Debug("Recv an audio RTP package")
					timer = time.Now()
				}
			...
			case session.vRTPChannel:
				...
				elapsed := time.Now().Sub(timer)
				if elapsed >= 30*time.Second {
					//beego.Debug("Recv an video RTP package")
					timer = time.Now()
				}
			...
			default:
				beego.Debug("unknow rtp pack type, %v", pack.Type)
				continue
			}
			if pack == nil {
				beego.Debug("session tcp got nil rtp pack")
				continue
			}
					...
					if len(line) == 0 {
						...
						if contentLen > 0 {
							...
							if n, err := io.ReadFull(session.connRW, bodyBuf); err != nil {
								...
								beego.Debug("read rtsp request body failed, expect size[%d], got size[%d]", contentLen, n)
								return
							}
...
func CheckAuth(authLine string, method string, sessionNonce string) error {
	...
	*/
	return fmt.Errorf("这里面有相互依赖不知道如何做.")
}
...
func (session *Session) handleRequest(req *Request) {
	...
	defer func() {
		if p := recover(); p != nil {
			beego.Debug("handleRequest err ocurs:%v", p)
			res.StatusCode = 500
		...
		if res.StatusCode != 200 && res.StatusCode != 401 {
			beego.Debug("Response request error[%d]. stop session.", res.StatusCode)
			session.Stop()
		}
	}()
	...
	case "ANNOUNCE":
		...
		req_url := req.URL
		if session.Server.handleRecvFilter != nil {
			ret_url, err := session.Server.handleRecvFilter(req_url)
			if err != nil {
				res.StatusCode = 406
				res.Status = "Not Acceptable:" + err.Error()
				return
			}
			req_url = ret_url
		}
		...
		if err != nil {
			res.StatusCode = 500
			res.Status = "Invalid URL 1:" + req.URL
			return
		}
	...
	case "DESCRIBE":
		...
		if err != nil {
			res.StatusCode = 500
			res.Status = "Invalid URL 2:" + req.URL
			return
		}
		session.Path = url.Path
		...
		session.Player, _ = NewPlayer(session, pusher)
		session.Pusher = pusher
	...
	case "SETUP":
		...
		if err != nil {
			res.StatusCode = 500
			res.Status = "Invalid URL 3:" + req.URL
			return
		}
		if setupUrl.Port() == "" {
		...
		if tcpMatchs := mtcp.FindStringSubmatch(ts); tcpMatchs != nil {
			...
			if setupPath == aPath || aPath != "" && strings.LastIndex(setupPath, aPath) == len(setupPath)-len(aPath) {
				...
				beego.Debug("SETUP [TCP] got UnKown control:%s", setupPath)
			}
			beego.Debug("Parse SETUP req.TRANSPORT:TCP.Session.Type:%d,control:%s, AControl:%s,VControl:%s", session.Type, setupPath, aPath, vPath)
			...
			beego.Debug("Parse SETUP req.TRANSPORT:UDP.Session.Type:%d,control:%s, AControl:%s,VControl:%s", session.Type, setupPath, aPath, vPath)
			if setupPath == aPath || aPath != "" && strings.LastIndex(setupPath, aPath) == len(setupPath)-len(aPath) {
				...
				beego.Debug("SETUP [UDP] got UnKown control:%s", setupPath)
			}
		}

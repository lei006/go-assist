...
import (
	...
	"livertc/core/intfs"
	"livertc/core/utils"
	"livertc/core/utils/parser/h264"
	"os"
...
func (pusher *Pusher) Server() *RtspServer {
	if pusher.Session != nil {
...
func (pusher *Pusher) Logger() *logs.BeeLogger {
	if pusher.Session != nil {
...
func NewPusher(session *Session) (*Pusher, error) {
	pusher := &Pusher{
		Session:        session,
		RTSPClient:     nil,
		players:        make(map[string]*Player),
	...
	pub_id := strings.Replace(session.Path, "/", "", -1)
	pub_id = strings.Replace(pub_id, ".sdp", "", -1)

	publisher, err := application.GetDefaultApp().GetEngine().MakePublisher(pub_id, "rtsp-pusher")
	if err != nil {
		return nil, err
	}

	pusher.publisher = publisher
...
func (pusher *Pusher) bindSession(session *Session) error {
	...
	session.RTPHandles = append(session.RTPHandles, func(pack *RTPPack) {
		if session != pusher.Session {
			beego.Debug("Session recv rtp to pusher.but pusher got a new session[%v].", pusher.Session.ID)
			return
		}
		pusher.QueueRTP(pack)
	})
	session.StopHandles = append(session.StopHandles, func() {
		if session != pusher.Session {
			beego.Debug("Session stop to release pusher.but pusher got a new session[%v].", pusher.Session.ID)
			return
		}
		pusher.ClearPlayer()
		pusher.Server().RemovePusher(pusher)
...
func (pusher *Pusher) RebindSession(session *Session) bool {
	if pusher.RTSPClient != nil {
		pusher.Logger().Warn("call RebindSession[%s] to a Client-Pusher. got false", session.ID)
		return false
	}
	sess := pusher.Session
	pusher.bindSession(session)
...
func (pusher *Pusher) RebindClient(client *RTSPClient) bool {
	if pusher.Session != nil {
		pusher.Logger().Warn("call RebindClient[%s] to a Session-Pusher. got false", client.ID)
		return false
	}
...
func (pusher *Pusher) QueueRTP(pack *RTPPack) *Pusher {
	...
	if pack.Type == RTP_TYPE_VIDEO {
		...
		*/
		...
		newPack, err := pusher.h264Parser.UnmarshalRTP(pack.Buffer.Bytes())
		if err == nil && newPack != nil {
			//fmt.Println("=================> pusher:", pusher.SDPMap["video"].SpropParameterSets)
			if len(pusher.SDPMap["video"].SpropParameterSets) >= 2 {
				newPack.SPS = pusher.SDPMap["video"].SpropParameterSets[0]
				newPack.PPS = pusher.SDPMap["video"].SpropParameterSets[1]
				newPack.DealFrame()
			} else {
				beego.Warn("SpropParameterSets size < 2")
			}
			pusher.publisher.ExistVideo(true)

			pusher.publisher.PublishPacket(newPack)
		}

	}
	if pack.Type == RTP_TYPE_AUDIO {
		pusher.publisher.ExistAudio(true)
	}

	if data[12] == 06 && data[13] == 05 {
		utils.PrintBinStr(data, 30, "xxxxwww")
	} else {
		pusher.cond.L.Lock()
...
func (pusher *Pusher) Start() {
	...
	for !pusher.Stoped() {
		...
		if pack == nil {
			if !pusher.Stoped() {
				beego.Debug("pusher not stoped, but queue take out nil pack")
			}
			continue
		}
...
func (pusher *Pusher) Stop() {
	...
	pusher.publisher.Close("rtsp pusher close")
...
func (pusher *Pusher) AddPlayer(player *Player) *Pusher {
	...
	if _, ok := pusher.players[player.ID]; !ok {
		...
		go player.Start()
		beego.Debug("%v start, now player size[%d]", player, len(pusher.players))
	}
	pusher.playersLock.Unlock()
	return pusher
}
...
func (pusher *Pusher) RemovePlayer(player *Player) *Pusher {
	...
	beego.Debug("%v end, now player size[%d]\n", player, len(pusher.players))
	pusher.playersLock.Unlock()
	return pusher
}

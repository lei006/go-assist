...
func NewRTSPClient(server *RtspServer, rawUrl string, sendOptionMillis int64, agent string) (client *RTSPClient, err error) {
	...
	for _, media := range _sdp.Media {
		...
		case "video":
			...
			if client.TransType == TRANS_TYPE_TCP {
				...
				if err != nil {
					beego.Debug("Setup video err.%v", err)
					return err
				}
			...
			beego.Debug("Parse DESCRIBE response, VIDEO VControl:%s, VCode:%s, url:%s,Session:%s,vRTPChannel:%d,vRTPControlChannel:%d", client.VControl, client.VCodec, _url, session, client.vRTPChannel, client.vRTPControlChannel)
		...
		case "audio":
			...
			if client.TransType == TRANS_TYPE_TCP {
				...
				if err != nil {
					beego.Debug("Setup audio err.%v", err)
					return err
				}
			...
			beego.Debug("Parse DESCRIBE response, AUDIO AControl:%s, ACodec:%s, url:%s,Session:%s, aRTPChannel:%d,aRTPControlChannel:%d", client.AControl, client.ACodec, _url, session, client.aRTPChannel, client.aRTPControlChannel)
			resp, err = client.RequestWithPath("SETUP", _url, headers, true)
			if err != nil {
				return err
			}
...
func (client *RTSPClient) startStream() {
	...
	for !client.Stoped {
		...
		if err != nil {
			if !client.Stoped {
				beego.Debug("client.connRW.ReadByte err:%v", err)
			}
			return
		}
		switch b {
		case 0x24: // rtp
			...
			if err != nil {
				...
				if !client.Stoped {
					beego.Debug("io.ReadFull err:%v", err)
				}
				return
			}
			...
			default:
				beego.Debug("unknow rtp pack type, channel:%v", channel)
				continue
			}
			if pack == nil {
			...
			if client.debugLogEnable {
				...
				if elapsed >= 30*time.Second {
					beego.Debug("%v read rtp frame.", client)
					loggerTime = time.Now()
				}
			}
		...
		default: // rtsp
			...
			for !client.Stoped {
				...
				if err != nil {
					if !client.Stoped {
						beego.Debug("client.connRW.ReadLine err:%v", err)
					}
					return
				}
				if len(line) == 0 {
				...
				if strings.Index(s, "Content-Length:") == 0 {
					...
					if err != nil {
						if !client.Stoped {
							beego.Debug("strconv.Atoi err:%v, str:%v", err, splits[1])
						}
						return
					}
				}

...
func (s *UDPServer) Logger() *logs.BeeLogger {
	if s.Session != nil {
...
func (s *UDPServer) SetupAudio() (err error) {
	...
	if err := s.AConn.SetReadBuffer(networkBuffer); err != nil {
		beego.Debug("udp server audio conn set read buffer error, %v", err)
	}
	if err := s.AConn.SetWriteBuffer(networkBuffer); err != nil {
		beego.Debug("udp server audio conn set write buffer error, %v", err)
	}
	...
	go func() {
		...
		beego.Debug("udp server start listen audio port[%d]", s.APort)
		defer beego.Debug("udp server stop listen audio port[%d]", s.APort)
		timer := time.Unix(0, 0)
		for !s.Stoped {
			if n, _, err := s.AConn.ReadFromUDP(bufUDP); err == nil {
				elapsed := time.Now().Sub(timer)
				if elapsed >= 30*time.Second {
					beego.Debug("Package recv from AConn.len:%d\n", n)
					timer = time.Now()
				}
				...
				beego.Debug("udp server read audio pack error", err)
				continue
			}
		}
	}()
	...
	if err := s.AControlConn.SetReadBuffer(networkBuffer); err != nil {
		beego.Debug("udp server audio control conn set read buffer error, %v", err)
	}
	if err := s.AControlConn.SetWriteBuffer(networkBuffer); err != nil {
		beego.Debug("udp server audio control conn set write buffer error, %v", err)
	}
	...
	go func() {
		bufUDP := make([]byte, UDP_BUF_SIZE)
		beego.Debug("udp server start listen audio control port[%d]", s.AControlPort)
		defer beego.Debug("udp server stop listen audio control port[%d]", s.AControlPort)
		for !s.Stoped {
			if n, _, err := s.AControlConn.ReadFromUDP(bufUDP); err == nil {
				//beego.Debug("Package recv from AControlConn.len:%d\n", n)
				rtpBytes := make([]byte, n)
				s.AddInputBytes(n)
				copy(rtpBytes, bufUDP)
				pack := &RTPPack{
				...
				beego.Debug("udp server read audio control pack error", err)
				continue
			}
		}
	}()
...
func (s *UDPServer) SetupVideo() (err error) {
	...
	if err := s.VConn.SetReadBuffer(networkBuffer); err != nil {
		beego.Debug("udp server video conn set read buffer error, %v", err)
	}
	if err := s.VConn.SetWriteBuffer(networkBuffer); err != nil {
		beego.Debug("udp server video conn set write buffer error, %v", err)
	}
	...
	go func() {
		bufUDP := make([]byte, UDP_BUF_SIZE)
		beego.Debug("udp server start listen video port[%d]", s.VPort)
		defer beego.Debug("udp server stop listen video port[%d]", s.VPort)
		timer := time.Unix(0, 0)
		for !s.Stoped {
			if n, _, err := s.VConn.ReadFromUDP(bufUDP); err == nil {
				elapsed := time.Now().Sub(timer)
				if elapsed >= 30*time.Second {
					beego.Debug("Package recv from VConn.len:%d\n", n)
					timer = time.Now()
				}
				rtpBytes := make([]byte, n)
				s.AddInputBytes(n)
				...
				beego.Debug("udp server read video pack error", err)
				continue
			}
		}
	}()
	...
	if err := s.VControlConn.SetReadBuffer(networkBuffer); err != nil {
		beego.Debug("udp server video control conn set read buffer error, %v", err)
	}
	if err := s.VControlConn.SetWriteBuffer(networkBuffer); err != nil {
		beego.Debug("udp server video control conn set write buffer error, %v", err)
	}
	...
	go func() {
		bufUDP := make([]byte, UDP_BUF_SIZE)
		beego.Debug("udp server start listen video control port[%d]", s.VControlPort)
		defer beego.Debug("udp server stop listen video control port[%d]", s.VControlPort)
		for !s.Stoped {
			if n, _, err := s.VControlConn.ReadFromUDP(bufUDP); err == nil {
				//beego.Debug("Package recv from VControlConn.len:%d\n", n)
				rtpBytes := make([]byte, n)
				s.AddInputBytes(n)
				copy(rtpBytes, bufUDP)
				pack := &RTPPack{
				...
				beego.Debug("udp server read video control pack error", err)
				continue
			}
		}
	}()
